...
**当前实际观察（可自己用上面的命令核对）**：`chain-head/` 有 35 个按日链头（2026-08-22 起），`seq` 从 72 单调增长到 250（最后一条入链写入 2026-09-25T16:10:01+08:00）。数据摆在这里，看起来是什么样就是什么样。

## 变更记录

- **2026-09-25 · chain-head 文件去掉内部文档摘要块。** 此前的每日链头文件在本行之外
  还附带一份内部文档摘要块（若干内部文档的 sha256）。该块**对本仓库承诺的可验证性不起任何作用**
  —— 验证只需要 `seq` / `head` / 时间戳，而这三项逐字节未变；它属于不应公开的内部信息。
  故自 2026-09-25 起移除该块，**只保留 `seq` / `head` / 时间戳**。
  历史提交里仍留有旧格式（不移除历史：改写历史会破坏本仓库赖以立身的可追溯性）。
  想核对这次改动本身，比较 2026-09-25 前后两个提交的同一批文件即可 —— `seq` 与 `head` 必须完全一致。
